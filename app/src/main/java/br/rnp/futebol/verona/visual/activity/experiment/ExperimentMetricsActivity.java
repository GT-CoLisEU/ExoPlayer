...
//        }
    ...
    private void refreshListFromAuxiliar() {
        ArrayList<Integer> ids = new ArrayList<>();
        metrics.clear();
        if (experimentAux.getObjectiveQoeMetrics() != null)
            ids.addAll(experimentAux.getObjectiveQoeMetrics());
        if (experimentAux.getQosMetrics() != null)
            ids.addAll(experimentAux.getQosMetrics());
        lvAdapter.clear();
